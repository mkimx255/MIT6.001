...
class Message(object):
    ...
    def build_shift_dict(self, shift):
        ...
        for alpha in alphabet_list:
            if alpha.islower():
                shifted_list.append(chr((ord(alpha) - ord('a') + shift) % 26 + ord('a')))
            else:
                shifted_list.append(chr((ord(alpha) - ord('A') + shift) % 26 + ord('A')))
        shift_dict = dict(zip(alphabet_list, shifted_list))
        return shift_dict
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        for i in range(26):
            count = 0
            for word in list(super().apply_shift(i).split(' ')):
...
"""
def decrypt_story():
    ans = CiphertextMessage(get_story_string())
    return ans
